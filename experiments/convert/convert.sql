...
CREATE TABLE lineorder (
    ...
    orderdate           INTEGER REFERENCES date(datekey),
    orderpriority       CHAR(15),
    shippriority        CHAR(1),
    quantity            SMALLINT,
    extendedprice       INTEGER,
    ordtotalprice       INTEGER,
    discount            SMALLINT,
    revenue             INTEGER,
    supplycost          INTEGER,
    tax                 SMALLINT,
    commitdate          INTEGER REFERENCES date(datekey),
    shipmode            CHAR(10),
...
ALTER TABLE 
    lineorder 
DROP CONSTRAINT 
    lineorder_custkey_fkey, 
DROP CONSTRAINT
    lineorder_partkey_fkey,
DROP CONSTRAINT
    lineorder_suppkey_fkey,
DROP CONSTRAINT
...
ALTER TABLE
    lineorder
ADD CONSTRAINT
    lineorder_custkey_fkey FOREIGN KEY (custkey) REFERENCES new_customer(c_custkey),
ADD CONSTRAINT
    lineorder_partkey_fkey FOREIGN KEY (partkey) REFERENCES new_part(p_partkey),
ADD CONSTRAINT
    lineorder_suppkey_fkey FOREIGN KEY (suppkey) REFERENCES new_supplier(s_suppkey),
ADD CONSTRAINT
    lineorder_orderdate_fkey FOREIGN KEY (orderdate) REFERENCES day(d_daykey),
ADD CONSTRAINT
