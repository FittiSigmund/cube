...
class LevelMember:
        ...
        if self._children:
            ...
            if children:
                for child in children:
                    lm_name = child[0]
                    lm = LevelMember(lm_name, self._level.child, self._metadata, self._engine, self)
                    setattr(self._level.child, str(lm_name), lm)
    ...
    def __getattr__(self, item):
        result = getattr(self._level.child, item)
        if result:
            return result
        else:
    ...
    def _get_select_stmt_for_getattr_or_getitem(self):
    ...
    def _get_query_with_parents(self, attribute, parents):
        select_stmt = self._get_select_stmt_for_getattr_or_getitem()
        from_stmt = self._get_from_stmt_for_getattr_or_getitem()
    ...
    def _fetch_level_member_from_db(self, attribute, parents):
        if parents:
