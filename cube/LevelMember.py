...
class LevelMember:
    ...
    def __getattr__(self, item) -> LevelMember:
        result: LevelMember = getattr(self._level.child, item)
        if result:
            result.parent = self
            return result
        else:
    ...
    def _get_attr_or_item(self, item: Union[str, int]) -> LevelMember:
        ...
        if len(result) == 1:
            ...
            level_member: LevelMember = LevelMember(level_member_name, self._level.child, self._metadata, self._engine, self)
